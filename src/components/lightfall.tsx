...
const prepColors = (input: string[]) => {
  ...
  const base = (input.length ? input : fallback).slice(0, MAX_COLORS);
...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const callIfFn = (obj: object | null, key: "remove" | "destroy") => {
        const method = obj ? (obj as { [K in "remove" | "destroy"]?: unknown })[key] : null;
  ...
  return (
    <div
      ...
      style={
        (mixBlendMode ? { mixBlendMode } : {}) as CSSProperties
      }
    />
  );
}
